...
try {
  ...
  if (!pendingResult.handled || !pending || pending.intentType !== "create_watch_by_name") {
    throw new Error(`Expected natural monitor text to create pending intent, got result=${JSON.stringify(pendingResult)} pending=${JSON.stringify(pending)}.`);
  }
  if (!sentMessages.at(-1)?.includes("请确认") || !sentMessages.at(-1)?.includes("硅谷101")) {
  ...
  const watches = repositories.listWatchesForWorkspace(workspace.id).filter((watch) => watch.name === "硅谷101" && watch.query === "硅谷101");
